...
def questions_by_tag(request, tag, page='1'):
    ...
    return render(request, 'question_by_tag.html', {'tag': tag, 'questions': questions, 'tags': randomTags(Tag), 'users': randomUsers(User)})
...
def questions_by_user(request, user, page='1'):
    ...
    return render(request, 'question_by_user.html', {'user': user, 'questions': questions, 'tags': randomTags(Tag), 'users': randomUsers(User)})
...
def question(request, id):
    ...
    if request.method == "POST":
        ...
        form=AnswerForm()
    return render(request, 'question.html', {'question': question_, 'answers': answers, 'tags': randomTags(Tag), 'users': randomUsers(User), 'form': form})


###########################
def login(request):
    # return render(request, 'login.html', {'tags': randomTags(Tag), 'users': randomUsers(User), 'isAuthorized' : 0 })
